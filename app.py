...
show_pages(
    [
        Page("app.py", "Dashboard", "✈️"),
        Page("pages/2_Data.py", "Data Exploration", "🔍"),
        Page("pages/3_Modeling.py", "Modeling", "🤖"),
    ]
)
...
filter_1, filter_2, filter_3, filter_4, filter_5 = st.columns(
...
with filter_1:
    airline_filter = st.multiselect(
        label="Select Airline", options=list(airline_options))

with filter_2:
    origin_filter = st.multiselect(
        label="Select Origin", options=list(origin_options))

with filter_3:
    destination_filter = st.multiselect(
        label="Select Destination", options=list(dest_options))
...
with r_c1:
    ...
    with r2_c1:
        st.plotly_chart(functions.pie_chart_delayed_vs_non_delayed(
            df_q_91), use_container_width=True)
    with r2_c2:
        st.plotly_chart(functions.by_airline(
            df_q_91), use_container_width=True)

    with r3_c1:
        st.plotly_chart(functions.by_origin(
            df_q_91), use_container_width=True)
    with r3_c2:
        st.plotly_chart(functions.by_dest(
            df_q_91), use_container_width=True)
